...
function applyCourse(courseId, grade, cls, name){
    ...
    if(course.students.length>=course.limit){ alert("정원 마감"); return; }
...
function addStudentManual(){
    ...
    currentAdminCourse.students.push({grade,className:cls,name:name,time:new Date().toLocaleString()});
    saveCourses();
    renderStudentList();
    alert("학생 추가 완료!");
}
...
// ---------- 관리자 공용 ----------
function logoutAdmin(){ alert("로그아웃"); location.href="admin-login.html"; }
